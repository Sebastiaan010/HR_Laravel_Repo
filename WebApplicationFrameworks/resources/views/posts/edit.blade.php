<x-layout>
  ...
  <form method="POST" action="{{ route('posts.update',$post) }}" enctype="multipart/form-data" class="max-w-xl space-y-3">
    @csrf @method('PUT')
    ...
    <div>
      ...
      <textarea name="body" rows="6" class="border rounded w-full px-3 py-2">{{ old('body',$post->body) }}</textarea>
    </div>

    <div>
      <label class="block text-sm mb-1">Afbeelding (optioneel)</label>
      @if($post->image_path)
        <img src="{{ asset('storage/'.$post->image_path) }}" class="w-40 h-28 object-cover border rounded mb-2" alt="">
      @endif
